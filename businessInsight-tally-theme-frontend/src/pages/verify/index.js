...
const VerifyPage = () => {
    // ** State
    const [authToken, setAuthtoken] = useState(useSelector(setAuthToken))
    ...
    useEffect(() => {
        try {
            if (verifyChangeEmailData && !verifyChangeEmailLoading) {
                router.push('/login');
                enqueueSnackbar(verifyChangeEmailData?.message, { variant: 'success' });
            }
        ...
        <Box className='content-center'>
            <Card sx={{ zIndex: 1 }}>
                <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
                    ...
                    <Box sx={{ mb: 6 }}>
                        <Typography sx={{ display: 'flex', justifyContent: 'center', marginBottom: 3 }} variant='body'>Please Wait...</Typography>
                        <Box sx={{ display: 'flex', justifyContent: 'center' }} variant='body'>
                            <CircularProgress />
                        </Box>
                    </Box>
                </CardContent>
